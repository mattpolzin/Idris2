...
newEntry n ctxt
    = do let idx = nextEntry ctxt
         ...
         pure (idx, { nextEntry := idx + 1,
                      ...
                      possibles $= addPossible n idx} ctxt)
...
clearCtxt
    = do defs <- get Ctxt
         let opts = defs.options
         resetDefs <- initDefs
         put Ctxt (updateOptions ({ timings := defs.timings } resetDefs) (const (resetElab opts)))
  where
...
setExtension e
    = do defs <- get Ctxt
         put Ctxt (updateOptions defs (setExtension e))
...
setPair fc ty f s
    = do defs <- get Ctxt
         ...
         f' <- checkUnambig fc f
         s' <- checkUnambig fc s
         put Ctxt (updateOptions defs (setPair ty' f' s'))
...
setRewrite fc eq rw
    = do defs <- get Ctxt
         ...
         put Ctxt (updateOptions defs (setRewrite eq' rw'))
...
setFromInteger n
    = do defs <- get Ctxt
         put Ctxt (updateOptions defs (setFromInteger n))
...
setFromString n
    = do defs <- get Ctxt
         put Ctxt (updateOptions defs (setFromString n))
...
setFromChar n
    = do defs <- get Ctxt
         put Ctxt (updateOptions defs (setFromChar n))
...
setFromDouble n
    = do defs <- get Ctxt
         put Ctxt (updateOptions defs (setFromDouble n))
...
setSession sopts
    = do defs <- get Ctxt
         put Ctxt (updateOptions defs ({ session := sopts }))
    ...
    = do s <- getSession
         ...
         when (cg `elem` s.incrementalCGs) $
           case lookup cg inc of
                ...
                Just (mods, extra) =>
                     put Ctxt ({ allIncData $= addMod cg (mods, extra) } defs)
  where
